...
class OnboardingInterviewViewModel: ObservableObject {
    ...
    func submitResponse() {
        ...
        print("DEBUG: Submitting response: \(currentUserInput)")
        print("DEBUG: Current question index: \(currentQuestionIndex)")
        ...
        print("DEBUG: New question index: \(currentQuestionIndex)")
        print("DEBUG: Total questions: \(questions.count)")
    ...
    private func processResponseAndContinue(_ response: String) async {
        print("DEBUG: Processing response - question index: \(currentQuestionIndex)")
        ...
        if currentQuestionIndex < questions.count {
            if currentQuestionIndex == questions.count - 1 {
                print("DEBUG: Generating final analysis message")
                // Final analysis message
                await generateAnalysisMessage()
            } else {
                print("DEBUG: Asking next question")
                await askNextQuestion()
            }
        } else {
            print("DEBUG: Completing interview")
            completeInterview()
        }
    }
